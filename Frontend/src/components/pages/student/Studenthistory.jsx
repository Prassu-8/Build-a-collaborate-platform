...
const Studenthistory = () => {
 ...
 const history =async()=>{
  try{
    await AxiosApi.get(`/student/history/${student._id}`).then((responce)=>{
      console.log(responce)
      setdata(responce.data.history)
    })
  ...
  <table className="w-full">
    ...
    <tbody className="bg-white">
      ...
      <tr className="text-gray-700" key={item._id}>
        ...
        <td className="px-4 py-3 text-sm border">{item.organized}</td>
        ...
        <td className=" flex space-x-3 p-4 border"> <Link to={`/givefeedback/${item._id}`} >Feedbacks</Link></td>       
