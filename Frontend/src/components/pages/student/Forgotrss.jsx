...
const Forgotrss = () => {
    const {handleSubmit:handleSubmit8 , register:register8} = useForm();
    const NavigateTo = useNavigate()
    const forgotpassword = async(data) =>{
        try{
            const response = await AxiosApi.put('/student/forgotrss',data);
            console.log(response);
            const successfullmessage = response.data.message;
            toast.success(successfullmessage);
            NavigateTo('/rsslogin')
  ...
  return (
    <div>
      <div className="bg-blue-950 min-h-screen w-screen flex items-center justify-center fixed top-0">
            <Link to="/rsslogin" className="absolute top-10 left-10 text-slate-50 font-bold">
            ...
            <div className="flex flex-col items-center">
                <form
                    ...
                    onSubmit={handleSubmit8(forgotpassword)}
                >
