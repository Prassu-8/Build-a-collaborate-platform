...
import Carousels from '../../components/pages/Faculty/Curosels';
...
const Navbar = () => {
  ...
  const getProfile = async() =>{
    try{
      await AxiosApi.get(`/faculty/profile/${faculty._id}`).then((response)=>{
  ...
  const handleSearch = async () => {
      try {
          const response = await AxiosApi.get(`/faculty/serachevent/${faculty._id}`, {
              params: { name: search }
          });
     ...
     <Carousels/>
      ...
      <div className={`fixed top-20 left-0 h-screen bg-sky-950 border-spacing-2 border-slate-50 w-64 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-all duration-300 ease-in-out`}>
          ...
          <div className=" flex" onMouseOver={toggleDropdown} onMouseOut={toggleDropdown}>
            ...
            {isDropdownOpen && (
              <div className="absolute bg-white w-48 mt-8 p-2 rounded-md shadow-lg">
                ...
                <Link to="/viewstudent" className="block text-black hover:text-blue-500">View Students</Link>
              </div>
            )}
          </div>
          <div className=" flex" onMouseOver={tooggleeventDropdown} onMouseOut={tooggleeventDropdown}>
            ...
            {isEventdropdown && (
              <div className="absolute bg-white w-48 mt-8 p-2 rounded-md shadow-lg">
                ...
                <Link to="/viewevent" className="block text-black hover:text-blue-500">View upcome events</Link>
              </div>
            )}
          </div>
          <div className=" flex">
          <i className="fa-solid fa-clock-rotate-left text-2xl text-white ml-4 mt-1"></i>
            <Link to='/history' className='text-justify text-xl text-white p-2 font-sans'>History</Link>
          </div>
        ...
        <div className=" ml-28 mt-7 space-y-3">
          ...
          <Link to='/profile' className=' w-20 h-12 rounded-lg  text-slate-950 bg-slate-300 '> View Profile</Link>
        </div>)}
